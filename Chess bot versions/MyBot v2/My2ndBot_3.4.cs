...
public class MyBot : IChessBot
{
    ...
    // Phase weights for tapering (Non-pawn/Non-king material)
    ...
    private const int TotalPhase = 24; // 4*1 + 4*1 + 4*2 + 2*4                                     // Best: ?
    ...
    private int Evaluate(Board board)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            // Piece-Square Table & Material Scoring
            while (whiteBB != 0)
            {
                int sq = BitOperations.TrailingZeroCount(whiteBB);
                if (i == 2) whiteBishopCount++;

                mgScore += MG_PieceValues[i] + PiecePSTs[i][sq];
                // Use MG PSTs for EG except for King (which has a dedicated EG PST)
                int egPst = (i == 5) ? KingEndGamePST[sq] : PiecePSTs[i][sq];
                egScore += EG_PieceValues[i] + egPst;

                whiteBB &= whiteBB - 1;
            }
            while (blackBB != 0)
            {
                int sq = BitOperations.TrailingZeroCount(blackBB);
                int mirrorSq = sq ^ 56;
                if (i == 2) blackBishopCount++;

                mgScore -= MG_PieceValues[i] + PiecePSTs[i][mirrorSq];
                int egPst = (i == 5) ? KingEndGamePST[mirrorSq] : PiecePSTs[i][mirrorSq];
                egScore -= EG_PieceValues[i] + egPst;
        ...
        // Pawn Structure (Simplified integration)
        int pawnMg, pawnEg;
        CalculatePawnScore(board, out pawnMg, out pawnEg);
        mgScore += pawnMg;
        egScore += pawnEg;

        // Tapering: Smoothly interpolate between MG and EG
        // Ensure phase doesn't exceed TotalPhase (e.g., due to promotions)
        int phase = Math.Min(gamePhase, TotalPhase);
        int finalScore = (mgScore * phase + egScore * (TotalPhase - phase)) / TotalPhase;

        // Tempo bonus and perspective
