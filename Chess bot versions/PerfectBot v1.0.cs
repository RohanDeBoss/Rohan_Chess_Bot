...
// PerfectBot v1.0: A tactically perfect searcher with no speculative pruning.
public class PerfectBot : IChessBot
{
    // --- Configuration ---
    private static readonly bool PerMoveDebugging = false;
    ...
    // Move Ordering Bonuses (Crucial for Alpha-Beta efficiency)
    ...
    private int Negamax(Board board, int depth, int alpha, int beta, int ply, int realPly)
    {
        ...
        // --- All speculative pruning is removed (NMP, Futility, etc.).
        // --- The variables 'standPat', 'inCheck', and 'inMateZone' are now unused and removed for performance.
        ...
        for (int i = 0; i < moves.Length; i++)
        {
            ...
            // Check Extensions are a safe way to deepen the search on forcing lines.
            if (givesCheck && newDepth < MaxSafetyDepth - 1) newDepth++;

            // Unconditional full-depth search for every move.
    ...
    private int Quiescence(Board board, int alpha, int beta, int ply, int qDepth, int realPly)
    {
        ...
        {
            // --- REMOVED: SEE-based pruning. All captures are searched. Ordering remains for efficiency.
    ...
    private (PieceType type, Square fromSquare) GetLeastValuableAttacker(Board board, Square targetSquare, bool attackerIsWhite, ulong occupiedHypothetical)
    {
        for (int pieceTypeId = 1; pieceTypeId <= 6; pieceTypeId++)
        {
            ...
            if (actualAttackers != 0) return (currentPieceTypeToTest, new Square(BitOperations.TrailingZeroCount(actualAttackers)));
        }
        return (PieceType.None, default(Square));
    }
    private void DebugLog(string message) => Console.WriteLine($"{GetType().Name} {message}");
    private Move LogEval(Board board, int depthCompleted, bool isForcedMove, Move moveForThisTurn)
    {
        if (PerMoveDebugging)
        {
            if (isForcedMove) Console.WriteLine($"\n{GetType().Name}: FORCED MOVE! ({moveForThisTurn})");
    ...
    {
        if (Math.Abs(score) <= CHECKMATE_SCORE_THRESHOLD) return null;
    ...
    private void AddTT(ulong key, int depth, short scoreForTT, byte flag, Move bestMove)
    {
        ...
        if (replace)
        {
            entry.Key = key; entry.Depth = (short)depth; entry.Score = scoreForTT; entry.Flag = flag;
    ...
    {
        if (Math.Abs(scoreFromNode) > CHECKMATE_SCORE_THRESHOLD) return (short)Math.Clamp(scoreFromNode, -(InfiniteScore - MAX_KILLER_PLY * 50), InfiniteScore - MAX_KILLER_PLY * 50);
