...
// Trustbot 1.3 - King Attack HCE + Knight Centrality + Endgame King Proximity
// Added: If enemy has < 10 points of material, penalize our King for being far away.
//        Penalty: -0.05 per orthogonal step (Manhattan distance) > 2.
public class MyBot : IChessBot
{
    ...
    private const int ENDGAME_MAT_THRESHOLD = 1000; // 10 points (P=100)
    ...
    private int Evaluate(Board board)
    {
        ...
        // Closest legal distance is generally 2 (e.g. adjacent diagonals or 1 orthogonal gap)
        // Penalty starts accruing if distance > 2
        int distancePenalty = Math.Max(0, (manhattanDist - 2) * KING_PROXIMITY_PENALTY);

        if (distancePenalty > 0)
        {
            // If Black is low on material, White wants to be close. Penalize White for being far.
            if (blackMaterial < ENDGAME_MAT_THRESHOLD)
            {
                mgScore -= distancePenalty;
                egScore -= distancePenalty;
            }
            // If White is low on material, Black wants to be close. Penalize Black for being far.
            // Since Black score is negative, we Add to make it less negative (worse for Black).
            if (whiteMaterial < ENDGAME_MAT_THRESHOLD)
            {
                mgScore += distancePenalty;
                egScore += distancePenalty;
            }
