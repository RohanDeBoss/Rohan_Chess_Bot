...
// Trustbot 1.5 - Fixes Endgame Proximity Bug
// Fix: King proximity bonus now checks (myMaterial > oppMaterial) to prevent
//      cancellation in K+Q vs K endgames where both are < 1000 pts.
// Includes: Bishop Mobility, King Attack, Knight Centrality.
public class MyBot : IChessBot
{
    ...
    private int Evaluate(Board board)
    {
        ...
        {
            // If Black is low on material AND White has more material, White wants to be close.
            if (blackMaterial < ENDGAME_MAT_THRESHOLD && whiteMaterial > blackMaterial)
            {
                mgScore -= distancePenalty;
                egScore -= distancePenalty;
            }
            // If White is low on material AND Black has more material, Black wants to be close.
