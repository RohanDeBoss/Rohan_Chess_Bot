...
// Trustbot 1.9 - King Safety (Pseudo-Queen)
// Added: MG King Safety.
//        1. Castle Bonus: +0.10 if King is on files A,B,G,H.
//        2. Exposure Penalty: Treat King as Queen (Diagonals + Verticals ONLY).
//           - Penalty (-0.02) per legal move found.
//           - Horizontal rays ignored (allows rook connection).
//        - Applies only to MG Score (fades out in endgame).
public class MyBot : IChessBot
{
    ...
    // King Safety (MG Only)
    private const int KING_SAFETY_FILE_BONUS = 10;      // +0.10 for being on side files
    ...
    private int Evaluate(Board board)
    {
        ...
        if (wFile < 2 || wFile > 5) mgScore += KING_SAFETY_FILE_BONUS;
        ...
        if (bFile < 2 || bFile > 5) mgScore -= KING_SAFETY_FILE_BONUS;
