...
// Trustbot 1.7 - Rook Activity
// Added: Rook Open/Semi-Open File bonuses & Mobility.
//        - Weighted heavily towards EG (End Game) score to prevent early game recklessness.
// Previous: Bishop Mobility, King Attack, Knight Centrality, Endgame King Proximity/Edge.
public class MyBot : IChessBot
{
    ...
    private const int BISHOP_MOBILITY_BONUS = 1;    // +0.01

    // Rook HCE (Split MG/EG to target late game)
    private const int ROOK_MOBILITY_MG = 1;         // +0.01 per square
    private const int ROOK_MOBILITY_EG = 3;         // +0.03 per square
    private const int ROOK_OPEN_FILE_MG = 10;       // +0.10
    private const int ROOK_OPEN_FILE_EG = 25;       // +0.25
    private const int ROOK_SEMI_OPEN_FILE_MG = 5;   // +0.05
    private const int ROOK_SEMI_OPEN_FILE_EG = 15;  // +0.15

    // Positional Bonuses
    private static readonly int[] KnightCenterBonus = { 0, 5, 15, 25 };
    private static readonly int[] KingCenterBonus = { 0, 10, 20, 30 };

    // Endgame King Proximity
    private const int ENDGAME_MAT_THRESHOLD = 1000; // 10 points
    private const int KING_PROXIMITY_PENALTY = 5;   // -0.05 per dist
    ...
    private int Evaluate(Board board)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            while (tempWhite != 0)
            {
                ...
                if (i == 3)
                {
                    ...
                    mgScore += moveCount * ROOK_MOBILITY_MG;
        ...
        // 2. Centrality
        int wFile = whiteKingSq.File; int wRank = whiteKingSq.Rank;
        int wDist = Math.Min(Math.Min(wFile, 7 - wFile), Math.Min(wRank, 7 - wRank));
        egScore += KingCenterBonus[wDist];

        int bFile = blackKingSq.File; int bRank = blackKingSq.Rank;
        int bDist = Math.Min(Math.Min(bFile, 7 - bFile), Math.Min(bRank, 7 - bRank));
    ...
    {
        mg = 0; eg = 0;
        // whitePawns and blackPawns are re-calculated here for isolation/passed logic
        ulong whitePawns = board.GetPieceBitboard(PieceType.Pawn, true);
        ulong blackPawns = board.GetPieceBitboard(PieceType.Pawn, false);
