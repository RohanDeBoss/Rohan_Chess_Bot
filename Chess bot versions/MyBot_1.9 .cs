...
public class MyBot : IChessBot
{
    ...
    private void SetPrincipalVariation(Move move, int depth)
    {
    ...
    private List<Move> SortMoves(Board board, IEnumerable<Move> moves, int depth)
    {
        var sortedMoves = new List<Move>(moves);

        // Sort by principal variation moves first
        sortedMoves.Sort((a, b) =>
        {
            if (Array.IndexOf(principalVariation, a) < 0 && Array.IndexOf(principalVariation, b) < 0)
                return 0;

            if (Array.IndexOf(principalVariation, a) < 0)
                return 1;

            if (Array.IndexOf(principalVariation, b) < 0)
                return -1;
        ...
        sortedMoves.Sort((a, b) =>
        {
            int aScore = killerMoves.TryGetValue(a, out int scoreA) ? scoreA : 0;
            int bScore = killerMoves.TryGetValue(b, out int scoreB) ? scoreB : 0;
        ...
        sortedMoves.Sort((a, b) =>
        {
            int aScore = history.TryGetValue(a, out int scoreA) ? scoreA : 0;
            int bScore = history.TryGetValue(b, out int scoreB) ? scoreB : 0;
            return bScore.CompareTo(aScore);
        });
