...
public class MyBot : IChessBot
{
    ...
    int Evaluate(Board board, int depth)
    {
        ...
        if (board.IsDraw())
        {
            return -25; // Neutral score for draw
        }
        ...
        if (whiteMaterial < 1700 || blackMaterial < 1700) // Endgame
        {
    ...
    {
        if (pawn == 0) return 0; // Handle case where there is no pawn on the given square

        // Get the index of the least significant bit set to 1, which is the pawn's position
        int squareIndex = BitOperations.TrailingZeroCount(pawn);

        // Calculate rank based on the square index and whether the pawn is white or black
        int rank = squareIndex / 8 + 1;

        // Adjust rank if the pawn is black, as ranks are mirrored for black
        if (!isWhite)
        {
            rank = 8 - rank + 1;
        }
