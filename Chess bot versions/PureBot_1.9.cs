...
//PureBot 1.0 A useful testing bot that should never miss tactics up to it's depth searched unless something is broken!
public class MyBot : IChessBot
{
    ...
    private int Negamax(Board board, int depth, int alpha, int beta, int ply, int realPly)
    {
        ...
        // Generate moves once for use in both depth 0 check and main search
        Move[] moves = board.GetLegalMoves();

        // At depth 0, check for immediate checkmate before quiescence
        if (depth <= 0)
        {
            foreach (Move move in moves)
            {
                ...
                {
                    board.UndoMove(move);
                    // Mate in one move for the side to move
                    return InfiniteScore - (realPly + 1) * 50;
                }
                board.UndoMove(move);
            }
            // No immediate mate, proceed to quiescence search
            return Quiescence(board, alpha, beta, ply);
        }

        // Check for no legal moves (stalemate or checkmate already handled)
        ...
        for (int i = 0; i < moves.Length; i++)
        {
            ...
            if (givesCheck) newDepth += 1; // Optional check extension
            ...
            {
                // Zero-window search for move ordering
