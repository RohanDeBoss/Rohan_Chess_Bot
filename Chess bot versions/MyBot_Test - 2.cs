...
public class MyBot : IChessBot
{
    ...
    public Move Think(Board board, Timer timer)
    {
        ...
        if (timerRef.MillisecondsRemaining < 7000)
        {
            maxTime = 400;
            depth = 4;
        }
    ...
    // Define the max quiescence depth (can also be made configurable if needed)
    const int maxQuiescenceDepth = 5;

    //Quiescence Search To Resolve Pieces Under Threat
    int QuiescenceSearch(int alpha, int beta, int color, int currentDepth)
    {
        // If currentDepth has reached the max quiescence depth, return the static evaluation
        if (currentDepth <= -maxQuiescenceDepth)
            return CalculatePosition(color, currentDepth);
