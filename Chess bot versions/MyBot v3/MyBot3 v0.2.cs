...
// MyBot v0.2 - PST Optimization
// Change: Integrated PSTs with loop hoisting for performance.
// logic: Replaced manual HCE bonuses with PST lookups.
...
public class MyBot : IChessBot
{
    ...
    // Index via PieceType: None, Pawn, Knight, Bishop, Rook, Queen
    // King is handled separately, so we don't need it here.
    ...
    private int Evaluate(Board board)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            ...
            // Pre-fetch PST array for this piece type if it's not the King
            int[] currentPST = (i != 5) ? PiecePSTs[i + 1] : null;
            ...
            while (tempWhite != 0)
            {
                ...
                if (i != 5) // Not King
                {
                    // Optimization: Use pre-fetched array
                    int pstVal = currentPST[pstIndex];
